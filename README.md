...
**Spin up a production-style Kubernetes cluster from one machine — via MCP (Model Context Protocol). Under the hood it's the [k8s-easy-install](https://github.com/ratulb/k8s-easy-install) project wrapped as an MCP server.**
...
`opencode.json` in the project root registers the MCP server automatically. OpenCode will load the following tools:

| Tool | Description |
|------|-------------|
| `write_config` | Write cluster config to `setup.conf` |
| `read_config` | Read current `setup.conf` |
| `create_cluster` | Build the full cluster |
| `destroy_cluster` | Full nuclear teardown |
| `get_cluster_status` | Nodes + pods status |
